...
def fetch_multi_timeframe(symbol, htf_1h_range='3mo', ltf_5m_range='5d'):
    """
    Fetches everything needed for the 4H→1H→15M→5M hierarchy:
        - 1H candles (refinement + resampled to 4H)
        - 4H candles (built by resampling 1H, 4 bars per group)
        - 15M candles (setup confirmation + SL anchor)
        - 5M candles (execution trigger only)
    """
    candles_1h = fetch_candles(symbol, interval='60m', range_=htf_1h_range)
    candles_4h = resample_candles(candles_1h, 4)
    candles_15m = fetch_candles(symbol, interval='15m', range_='5d')
