...
def should_execute(setup, config: ExecutionConfig, daily_state: DailyState, open_positions):
    ...
    # Score < 8:  max 1 position per symbol
    # Score 8-9:  max 2 positions per symbol
    # Score = 10: max 3 positions per symbol
    ...
    if same_symbol_count >= max_for_symbol:
        return False, f'Already at max positions ({max_for_symbol}) for {symbol} at score {score}'
